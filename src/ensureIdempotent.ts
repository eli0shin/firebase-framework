...
import { IdempotentEventHandler, EventHandler } from './types/Service';
...
export function withIdempotency(functionName: string, handler: IdempotentEventHandler): EventHandler {
  return async function ensureIdempotent(...args: Parameters<EventHandler>): ReturnType<EventHandler> {
    ...
    if (!('eventId' in changeContext)) {
      throw new Error('Received an event that cannot be made idempotent, context is missing eventId');
    }
    ...
    if (!eventDidRun.exists || !eventDidRun.data()?.completed) {
      ...
      const callback = async (t: Transaction) => {
        if (t) {
